...
class BaseIsActiveModel(models.Model):
    ...
    is_active_updated_by = models.ForeignKey(User, on_delete = models.CASCADE, related_name = "is_active_updated_%(class)ss", default = User.get_default_user, editable = False)
    is_active_updated_on = MonitorField(monitor = "is_active", editable = False)
